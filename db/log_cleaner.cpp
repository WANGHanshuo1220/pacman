...
void LogCleaner::CleanerEntry_0() {
...
void LogCleaner::CleanerEntry_12() {
  // bind_core_on_numa(log_->num_workers_ + cleaner_id_);
#if INDEX_TYPE == 3
  reinterpret_cast<MasstreeIndex *>(db_->index_)
      ->MasstreeThreadInit(log_->num_workers_ + cleaner_id_);
...
void LogCleaner::CleanerEntry_3() {
  // bind_core_on_numa(log_->num_workers_ + cleaner_id_);
#if INDEX_TYPE == 3
  reinterpret_cast<MasstreeIndex *>(db_->index_)
      ->MasstreeThreadInit(log_->num_workers_ + cleaner_id_);
...
bool LogCleaner::NeedCleaning_123() {
...
void LogCleaner::CompactSegment_0(LogSegment *segment) {
  ...
  while (p < tail) {
    ...
    is_garbage = IsGarbage(kv, num_old);
    ...
    if (!is_garbage) {
      ...
      le_helper.old_val = TaggedPointer(p, sz, num_old, 0);
...
void LogCleaner::CompactSegment_123(LogSegment *segment) {
  char *p = segment->get_data_start();
  char *tail = segment->get_tail();
  std::vector<char *> flush_addr;
  bool is_garbage;
...
void LogCleaner::DoMemoryClean_0() {
  ...
  for (auto it = to_compact_segments_.begin();
       it != to_compact_segments_.end(); it++, i++) {
    assert(*it);
...
#else
  CompactSegment_0(segment);
#endif
}

void LogCleaner::DoMemoryClean_12() {
  // printf("in domemoryclean, class_ = %d\n", class_);
  TIMER_START_LOGGING(pick_time_);
  LockUsedList();
  to_compact_segments_.splice(to_compact_segments_.end(),
  ...
  BatchCompactSegment(segment);
#else
  CompactSegment_123(segment);
#endif
}

void LogCleaner::DoMemoryClean_3() {
  // printf("in domemoryclean, class_ = %d\n", class_);
  TIMER_START_LOGGING(pick_time_);
  LockUsedList();
  to_compact_segments_.splice(to_compact_segments_.end(),
...
void LogCleaner::MarkGarbage_0(ValueType tagged_val) {
...
void LogCleaner::MarkGarbage_123(ValueType tagged_val) {
  TaggedPointer tp(tagged_val);
