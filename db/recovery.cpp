...
void LogCleaner::RecoverySegments() {
  ...
  int num_free_seg = 0;
  std::vector<std::queue<LogSegment *>> tmp_free_queue0;
  tmp_free_queue0.resize(num_channel);
  ...
  for (; i < num_segments[0]; i += num_cleaners) {
    ...
    if (seg->header_->status == StatusAvailable) {
      int ch = seg->get_channel();
      tmp_free_queue0[ch].push(seg);
      ++num_free_seg;
    } else {
...

  {
    ...
    for(int i = 0; i < num_channel; i++)
    {
      int num_free = tmp_free_queue0[i].size();
      while (!tmp_free_queue0[i].empty()) {
        log_->free_segments_class0[i].push_back(tmp_free_queue0[i].front());
        tmp_free_queue0[i].pop();
      }
      ...
      for (; i < num_segments[j]; i += 2) {
        ...
        else 
        {
            ...
            while (p < end) {
              ...
              ValueType val = TaggedPointer((char *)kv, sz, c, j);
              Slice key = kv->GetKey();
              ...
              if (le_helper.old_val != INVALID_VALUE) {
                ...
                MarkGarbage_recovery(le_helper.old_val);
              }
              p += sz;
              c++;
            }
          if (seg->header_->status == StatusUsing) {
    ...
    if (seg->header_->status != StatusAvailable) {
      ...
      while (p < tail) {
        ...
        if (val != real_val) {
          ...
          if(seg->get_class() <= 0) seg->MarkGarbage((char *)kv, sz);
          else seg->roll_back_map[c].is_garbage = true;
  ...
  for (; i < num_segments[0]; i += num_cleaners) {
    ...
    if (seg->header_->status == StatusAvailable) {
      ...
      while (p < end) {
        ...
        ValueType val = TaggedPointer((char *)kv, sz, 0, 0);
        Slice key = kv->GetKey();
        LogEntryHelper le_helper(val);
        db_->index_->Put(key, le_helper);
#ifdef GC_SHORTCUT
